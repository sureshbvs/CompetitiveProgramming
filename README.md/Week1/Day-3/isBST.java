...
public class Solution {
    ...
    private static boolean isBinarySearchTree(BinaryTreeNode root, int minimum, int maximum) {
        if (root == null) {
            return true;
        }
    
        if (root.value >= maximum || root.value <= minimum) {
            return false;
        }
    
        return isBinarySearchTree(root.left, minimum, root.value)
            && isBinarySearchTree(root.right, root.value, maximum);
    }
